...
export const DownloadsPage = () => {
  ...
  const statusConfig = {
    All:           { icon: FileDown,        color: 'teal' },
    Completed:     { icon: CheckCircle2,    color: 'emerald' },
    'In Progress': { icon: Loader,          color: 'amber' },
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
              ...
              <div className="relative flex h-36 w-36 items-center justify-center">
                ...
                <div className="relative flex flex-col items-center gap-3">
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/20 bg-white/15 shadow-md backdrop-blur-sm">
                      <FileDown className="h-5 w-5 text-teal-100" />
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          {statuses.map((status, idx) => {
            ...
            const colorMap = {
              ...
              red:     { bg: 'bg-red-50',     border: 'border-red-200',     activeBorder: 'border-red-500', text: 'text-red-700',     icon: 'text-red-500',     ring: 'ring-red-200' },
            };
            ...
            return (
              <button
                ...
                className={`heading-entrance rounded-xl border bg-white p-4 text-left shadow-sm transition-all duration-200 hover:shadow-md focus:outline-none ${isActive ? `${c.activeBorder} ring-2 ${c.ring}` : 'border-gray-200 hover:border-gray-300'}`}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {filteredDownloads.length > 0 ? (
          ...
          >
            {filteredDownloads.map((resource, idx) => (
              <DownloadCard key={resource.id} resource={resource} viewMode={viewMode} index={idx} />
            ))}
          </div>
        ) : (
        ...
        <>
          ...
          <div className="p-5 flex-1 flex flex-col">
            <div className="flex items-center justify-between mb-3">
              <span className="inline-block self-start text-xs font-semibold px-2.5 py-1 rounded-lg border border-teal-200 text-teal-700 bg-teal-50 shadow-sm">
                {resource.category}
              </span>
              <span className={`text-[0.65rem] uppercase tracking-wider font-bold px-2.5 py-1 rounded-lg border ${statusStyle}`}>
                {resource.status}
              </span>
            </div>
          ...
          <div className="flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-1 flex-wrap">
                ...
                <span className={`text-[0.65rem] uppercase tracking-wider font-bold px-2 py-0.5 rounded-lg border ${statusStyle}`}>
                  {resource.status}
                </span>
              </div>
